...
import {  useFavActions } from "../../Providers/FavProvider";
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className=" grid gap-x-2 gap-y-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-2 md:p-0 sm:gap-x-4 md:gap-x-6 md:gap-y-10">
        {games.map((game) => (
          <Product key={game.id} cart={cart} game={game} addProductHandler={addProductHandler} addProductToFavHandler={addProductToFavHandler} />
        ))}
      </div>
    </div>
  );
